...
遵循 [`Projects/schemas/log-entry.schema.json`](../schemas/log-entry.schema.json)：

- 每行 stdout 输出一条 JSON（`timestamp`、`level`、`service`、`message` 等）
- 同时 POST 到 Docker Vector `LOG_SINK_URL`（默认 `http://127.0.0.1:8088/`）→ Loki → Grafana

```bash
# 确保 Vector 已启动
cd ~/Docker && docker compose up -d vector grafana

# 验证
curl -X POST http://localhost:8088/ \
  -H 'Content-Type: application/json' \
  -d '{"timestamp":"2026-06-27T08:00:00.000Z","level":"info","service":"content-discovery","message":"test"}'
```

| 变量 | 默认 |
|------|------|
| `SERVICE_NAME` | `content-discovery` |
| `LOG_SINK_ENABLED` | `true` |
| `LOG_SINK_URL` | `http://127.0.0.1:8088/` |
| `LOG_LEVEL` | `info` |
